...
def test():
    ...
    for line in info:
        ...
        if (all != []):
            imagename = str(all[0]);
            if (os.path.isfile(r'../data/Non-Haze/%s.jpg' % imagename) == False):
                continue;
            else:
                imagesnum = imagesnum + 1;
                npstore = caffe.io.load_image('../data/Non-Haze/%s.jpg'% imagename)
                ...
                savepath = '../data/result_non-haze/' + imagename + '_AOD-Net.jpg'
